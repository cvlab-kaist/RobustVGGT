...
def load_and_preprocess_images(image_path_list, mode="crop"):
    ...
    for i, image_path in enumerate(image_path_list):
        ...
        # For any other mode, we just resize without padding/cropping
        if mode not in ["pad", "crop"]:
            # Fit other images sizes to the first image size by resizing
            if i > 0:
                first_img = images[0]
                new_width = first_img.shape[2]
                new_height = first_img.shape[1]
        ...
        # Record shape
        info_print(f"{image_path} shape after preprocessing: {img.shape}")
    ...
    if mode in ["pad", "crop"] and len(shapes) > 1:
    ...
    # print final shapes
    info_print(f"Final image shape after preprocessing: {images[0].shape}")
